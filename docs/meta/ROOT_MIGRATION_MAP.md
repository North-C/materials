...
迁移动作：并入硬件/架构主题入口（候选：`kvm/` 相邻的新笔记目录，或保留根级待 `每日项目简析/` 式主题页收编）。前置条件：用户确认是否值得保留为 backlog 占位（见"空与占位文件决策"）。
...
迁移动作：候选并入 `软件工程/`（现有 3 份方法论文档）。前置条件：补适用版本/环境边界后再定 canonical 状态。
...
对 `BDD与cucumber.md`、`golang_under_the_hood.md`、`不同芯片系列.md`、`指令集学习.md` 四份"无自有正文"文件，以及 `华泰问题讨论.md`：

1. **保留占位**：文件名代表明确的待写主题（BDD/cucumber、Go 内部机制）或待展开的参考集，保留在根级或迁入主题目录均可，但应在主题页标注 `placeholder`。
2. **补写后迁移**：保留至内容补齐，再按主题归类。
3. **归档**：若主题已被放弃，移入明确的归档位置（不删除）。

本映射不替用户选择；默认全部按 1 处理（保留 + 标注），任何删除都需要单独授权。

## 执行顺序建议

1. 用户先处置凭据风险（轮换两处模型 API key；`github-tokens.md` 按 MAT-05 既有流程）。
2. 用户审阅本映射，确认各 Group 的目标目录与空文件三态决策。
3. 获批后按 Group 逐个开独立 Workspace/commit 执行 Git rename：R1 → R4 → R5 → R6 → R2/R3（R1 内容最多、关联最强，优先收敛）。
4. 每组迁移后：更新 `docs/topics/root-files.md` 的映射、检查组内互链、记录新旧路径。
5. `日志.md` 的正文拆分放在凭据轮换完成之后，单独一批。
