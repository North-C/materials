#!/usr/bin/env python3
"""Summarize cube_bench_reusable.py results into a CSV file.

Input is a result directory produced by scripts/cube_bench_reusable.py.
...
def stdout_path(results_dir: Path, item: dict[str, Any]) -> Path:
    case = str(item["case"])
    result_dir = Path(item.get("result_dir") or results_dir)
    candidates = [
        result_dir / f"{case}.stdout",
        results_dir / case / f"{case}.stdout",
        results_dir / f"{case}.stdout",
    ]
...
def summarize(results_dir: Path, include_status: bool) -> list[dict[str, str]]:
    ...
    for item in summary:
        case = str(item["case"])
        path = stdout_path(results_dir, item)
        base = {
            "source": "envd",
            "results_dir": str(results_dir),
            "case": case,
            "sandbox_id": str(item.get("sandbox_id", "")),
            "sandbox_ip": str(item.get("sandbox_ip", "")),
            "exit_code": str(item.get("rc", "")),
        ...
        if include_status:
            add_metric(rows, base, "case", "exit_code", item.get("rc", ""), "code")
...
def main() -> int:
    ...
    parser.add_argument("--results-dir", required=True, help="Result directory produced by cube_bench_reusable.py.")
